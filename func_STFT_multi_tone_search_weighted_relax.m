function [IF,weights] = func_STFT_multi_tone_search_weighted_relax(signal,fs,window_dur,step_size_dur,fc,bound,FFT_res_factor)
...
band_high_signal_1st  = round(50.02*FFT_res_factor); 
band_low_noise_1st    = round(49.9*FFT_res_factor);
band_high_noise_1st   = round(50.1*FFT_res_factor);                
weights               = zeros(1,length(fc));
%% set harmonic search region
search_region_1st = round((50-bound(1)/2)*FFT_res_factor):round((50+bound(1)/2)*FFT_res_factor); 
